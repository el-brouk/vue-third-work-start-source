<template>
  <header class="header">
    ...
    <router-link v-if="authStore.getUserAttribute('isAdmin')" to="/tasks/create" class="header__create-task">
      Создать карточку
    </router-link>
    <!--    Аватар пользователя-->
    <a v-if="authStore.user" href="#" class="header__user" @click.stop="toggleUserMenu">
      <img :src="userImage" alt="Администратор" width="40" height="40" />
    </a>
    <!--    Панель пользователя-->
    <div v-if="isUserMenuOpened" v-click-outside="toggleUserMenu" class="header__menu">
      <div class="user-menu">
    ...
    <a v-if="!authStore.isAuthenticated" class="header__login" @click="$router.push('/login')"> Войти </a>
  </header>
</template>
