<template>
  <div class="task-card__check-list">
    <h4 class="task-card__title">
      ...
      <button v-if="!disabled" type="button" class="task-card__plus" @click="$emit('createTick')" />
    </h4>
    <!--Список подзадач-->
    <ul v-if="ticks.length" class="task-card__list">
      <li v-for="tick in ticks" :key="tick.id || tick.uuid" class="task-card__item">
        ...
        <div class="task-card__icons" :class="{ 'task-card__icons--hidden': disabled }">
          <app-icon class="icon--trash" @click="$emit('removeTick', { uuid: tick.uuid, id: tick.id })" />
        </div>
      </li>
    </ul>
  </div>
