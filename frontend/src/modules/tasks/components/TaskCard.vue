      ...
      <div class="task" @click="router.push({ path: `/${task.id}` })">
        ...
        <div v-if="task.user" class="task__user">
          <div class="task__avatar">
            <img :src="getPublicImage(taskUser.avatar)" alt="Аватар пользователя" width="20" height="20" />
          </div>
          {{ task.user.name }}
        </div>
        ...
        <h5 class="task__title" :class="{ 'task__title--first': !task.user }" data-test="task-title">
          {{ task.title }}
        </h5>
